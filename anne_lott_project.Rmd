---
title: "Modeling a Prognostic Indicator of Prostate Cancer"
...
According research conducted by the American Cancer Society, in 2018 around 165,000 men in the United States will be diagnosed with prostate cancer, the most common cancer in men after skin cancer (\textit{Key statistics}, 2018). If a patient is diagnosed with prostate cancer, the next step is to determine the prognosis, the likely result of the cancer in the near future. Arriving at an accurate prognosis helps the doctor create a treatment strategy, while also aiding the patient and his family with life planning given the disease. 
...
To model seminal vesicle invasion based on age of the patient and PSA level, I chose to use logistic regression, a classification method. Logistic regression models the probability of a binary random variable, such as the presence or absence of seminal vesicle invasion, given values of the predictors. Logistic regression modelling requires that the observations are independent and that the predictor variables are not linearly related (\textit{Assumptions}, 2018). I assume that the observations are independent because they come from different men, and I checked the correlation between age and PSA level to ensure that they were not linearly associated.
```{r, echo= FALSE}
#correlation between age and psa level
age_psa_r = cor(prostate_cancer$age, prostate_cancer$psa)
```
The correlation coefficient between age and PSA level was `r round(age_psa_r, 3)`, so there is no multicollinearity between these independent variables. 

Logistic regression also requires that the sample size is large (\textit{Assumptions}, 2018). I will train my logistic regression model on 67 observations, which I believe is a sufficiently large sample size. Finally, logistic regression necessitates that the log-odds of the response variable is linearly associated with the predictor variables (\textit{Assumptions}, 2018). I will check this assumption after I build my model, to see if the model is correct.

My preliminary logistic regression model, containing two predictors, PSA level and age, will have the following form:
$$P(Y = 1|X_1, X_2) = \frac{e^{\beta_0 + \beta_1X_1 + \beta_2X_2}}{1 + e^{\beta_0 + \beta_1X_1 + \beta_2X_2}}$$
where $Y$ is the incidence of seminal vesicle invasion (1 if yes, 0 if no), $X_1$ is the PSA level, $X_2$ is the patient's age, and $\beta_0$, $\beta_1$ and $\beta_2$ are the true coefficients. This model can also be written as 
$$\textrm{log}\Bigg(\frac{P(Y=1|X_1,X_2)}{1-P(Y=1|X_1,X_2)}\Bigg) = \beta_0 + \beta_1X_1 + \beta_2X_2$$
where $\textrm{log}\Big(\frac{P(Y=1|X_1,X_2)}{1-P(Y=1|X_1,X_2)}\Big)$ is the log-odds of seminal vesicle invasion. The model will be fit using the method of maximum likelihood in R (James et al., 2013).
...
```{r,echo = FALSE, fig.width = 10, fig.height = 3}
#test whether the relationship between the log-odds and the psa is linear
probs.training = predict(glm.svi, training_set, type = "response")
log_it = log(probs.training/(1-probs.training))
par(mfrow= c(1,2))
plot(training_set$psa, log_it, 
     xlab = "PSA Level", 
     ylab = "Log-odds of SVI", main= "Log-odds of SVI versus PSA")
#test whether the relationship between the log-odds and the age is linear
plot(training_set$age, log_it, xlab = "Age", 
     ylab = "Log-odds of SVI",
     main = "Log-odds of SVI versus Age")
```
...
\hline
With Age& \multicolumn{2}{|c|}{Actual SVI}\\
\hline
Predicted SVI &0&1\\
\hline
0&21&3\\
...
\end{tabular}
\end{center}
\caption{Confusion Matrix- Full Model}
}
\hfill
\parbox{.5\linewidth}{
\centering
...
\begin{tabular}{|p{1.5cm}|p{1.5cm}|p{1.5cm}|}
\hline
No Age & \multicolumn{2}{|c|}{Actual SVI}\\
\hline
Predicted SVI &0&1\\
\hline
0&19&4\\
...
\end{tabular}
\end{center}
\caption{Confusion Matrix- Reduced Model}
}
\end{table}
...
Creating a different training and test set at random two more times, fitting the full and reduced models to the training sets, and validating them on the test sets, I obtained similar results. The p-value for the coefficient for PSA level was always below 0.01 while the p-value for the age coefficient was always above 0.05. Tables 3 and 4 give the confusion matrices for the second randomly sampled test set, and tables 5 and 6 display the confusion matrices for the third randomly sampled test set.
...
For the second randomly sampled test set, again we see that for men with seminal vesicle invasion, only 50% are classified correctly by the model. For the third randomly sampled test set, men with seminal vesicle invasion are classified correctly in 66.67% of the cases. The overall accuracy rate of the full and the reduced model was 90% for the second test set and 76.67%  for the third test set. Since the full and reduced models have the same accuracy, I conclude that age has no predictive power here. 
...
\hangindent=1cm \textit{Assumptions of logistic regression}. \hspace{-2mm}(2018).\hspace{-2mm} Retrieved from https://www.statisticssolutions.com/assumptions-of-logistic-regression/

\hangindent=1cm James, G., Witten, D., Hastie, T., \& Tibshirani, R. (2013). \textit{An introduction to statistical learning with applications in R}. New York, NY: Springer.

\hangindent=1cm \textit{Key statistics for prostate cancer}. (2018). Retrieved from 
https://www.cancer.org/cancer/prostate-cancer/about/key-statistics.html

\hangindent=1cm Kutner M.H., Nachtsheim, C.J., \& Neter, J. (2004). \textit{Applied linear regression models} (4th ed.). Boston, MA: McGraw-Hill Irwin.

\hangindent=1cm Potter, S.R., Epstein, J.I., \& Partin, A.W. (2000). "Seminal vesicle invasion by prostate cancer: Prognostic significance and therapeutic implications." \textit{Reviews in Urology, 2(3),} 190-195.

\hangindent=1cm Stamey, T.A., Kabalin, J.N., McNeal, J.E., Johnstone, I.M., Freiha, F., Redwine, E.A., \& Yang, N. (1989). "Prostate specific antigen in the diagnosis and treatment of adenocarcinoma of the prostate. II. Radical prostatectomy treated patients."\textit{The Journal of Urology, 141,} 1076-1083.
